...
;
;   * We put the same reset routine (this routine, ResetMMC1_b5) at the start of
;     every ROM bank, so the same routine gets run, whichever ROM bank is mapped
;     to $C000.
;
; This ResetMMC1_b5 routine is therefore called when the NES starts up, whatever
...
IF _NTSC
 ...
 RTI                    ; Return from the IRQ interrupt without doing anything
                        ...
                        ;
                        ; Once bank 7 is switched into $C000 by the ResetMMC1_b5
...
IF _NTSC
 ...
 EQUW Interrupts_b5+$4000 ; Vector to the NMI handler in case this bank is loaded
...
IF _BANK = 5
 ...
 PRINT "S.bank5.bin ", ~CODE_BANK_5%, " ", ~P%, " ", ~LOAD_BANK_5%, " ", ~LOAD_BANK_5%
 SAVE "3-assembled-output/bank5.bin", CODE_BANK_5%, P%, LOAD_BANK_5%
